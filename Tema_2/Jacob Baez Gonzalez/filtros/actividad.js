...
function greyConverter() {
  ...
  //Aqui tu codigo
    ...
    for (let j = 0; j < pixels[i].length; j++) {
      ...
      let total = 0, media, item;

      for (item = 0; item < pixel.length; item++){
        total +=pixel[item]
      }
      media = total/pixel.length;
      pixel[0] = media; // Rojo a valor media
      pixel[1] = media; // Verde a valor media
      pixel[2] = media; // Azul a valor media
    }
  }
  handler.savePixels(pixels, outputPath);
}
...
function scaleDown() {
  ...
  for (let i = 0; i < pixels.length; i+=2) {
    ...
    for (let j = 0; j < pixels[i].length; j+=2) {
      ...
      if (j % 2 === 0) {
        let pixel = pixels[i][j];// Obténer el array de cada pixel 
        filaNueva.push(pixel);
      }

    }
    nuevaImg.push(filaNueva);
  }
  ...
  //Aqui tu codigo
   for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      ...
      for (item = 0; item < pixel.length; item++){
        brillo = Math.round(pixel[item]/dimFactor)
        pixel[item] = Math.max(0, Math.min(255, brillo)); //  brillo dimFactor
...
function invertColors() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      ...
      for (item = 0; item < pixel.length; item++){
        invert = 255-pixel[item]
        pixel[item] = Math.max(0, Math.min(255, invert)); //  invierte color
      }
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let i = 0; i < catPixels.length; i++) {
    ...
    for (let j = 0; j < catPixels[i].length; j++) {
      let pixelGato = catPixels[i][j];// Obténer el array de cada pixel gato
      let pixelPerro = dogPixels[i][j];// Obténer el array de cada pixel gato
      let pixelNuevo = [pixelGato[0] * alphaFirst + pixelPerro[0] * alphaSecond, 
                        pixelGato[1] * alphaFirst + pixelPerro[1] * alphaSecond,
                        pixelGato[2] * alphaFirst + pixelPerro[2] * alphaSecond];
      fusion.push(pixelNuevo);
      }
