...
function redConverter() {
  ...
  pixels.forEach((fila, i) => {
    fila.forEach((pixel, j) => {
      pixel[1] = 0;
      pixel[2] = 0;
    });
  });
...
function greyConverter() {
  ...
  pixels.forEach((fila, i) => {
    fila.forEach((pixel, j) => {
      const media = Math.floor((pixel[0] + pixel[1] + pixel[2]) / 3);
      pixel[0] = media;
      pixel[1] = media;
      pixel[2] = media;
    });
...
function scaleDown() {
  ...
  pixels = pixels.filter((fila, i) => i % 2 === 0).map(fila => fila.filter((pixel, j) => j % 2 === 0));
...
function merge(alphaFirst, alphaSecond) {
  ...
  dogPixels.forEach((fila, i) => {
    let newRow = [];
    fila.forEach((pixel, j) => {
      let newPixel = [];
      for (let k = 0; k < 3; k++) {
        let newValue = Math.floor(catPixels[i][j][k] * alphaFirst + dogPixels[i][j][k] * alphaSecond);
        newPixel.push(newValue);
      }
