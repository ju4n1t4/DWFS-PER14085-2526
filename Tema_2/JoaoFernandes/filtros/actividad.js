...
function greyConverter() {
  ...
  for (let i = 0; i < pixels.length; i++) {
    for (let j = 0; j < pixels[i].length; j++) {
      let avg = Math.floor((pixels[i][j][0] + pixels[i][j][1] + pixels[i][j][2]) / 3);
      pixels[i][j][0] = avg; 
      pixels[i][j][1] = avg; 
      pixels[i][j][2] = avg; 
    }
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let i = 0; i < dogPixels.length; i++) {
    ...
    for (let j = 0; j < dogPixels[i].length; j++) {
      let r = Math.min(Math.floor(dogPixels[i][j][0] * alphaFirst + catPixels[i][j][0] * alphaSecond), 255);
      let g = Math.min(Math.floor(dogPixels[i][j][1] * alphaFirst + catPixels[i][j][1] * alphaSecond), 255);
      let b = Math.min(Math.floor(dogPixels[i][j][2] * alphaFirst + catPixels[i][j][2] * alphaSecond), 255);
      newRow.push([r, g, b]);
    }
    pixels.push(newRow);
  }
