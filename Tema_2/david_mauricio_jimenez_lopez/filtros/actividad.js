...
function procesarPixel(pixels, filterFunction) {
  for (let fila = 0; fila < pixels.length; fila++) {
    for (let col = 0; col < pixels[fila].length; col++) {
      pixels[fila][col] = filterFunction(pixels[fila][col]);
    }
  }
  return pixels;
}
...
function merge(alphaFirst, alphaSecond) {
  ...
  let pixels = [];
  for (let fila = 0; fila < catPixels.length && fila < dogPixels.length; fila++) {
    let nuevaFila = [];
    for (let col = 0; col < catPixels[fila].length && col < dogPixels[fila].length; col++) {
      let nuevoPixel = [
        Math.round(catPixels[fila][col][0] * alphaFirst + dogPixels[fila][col][0] * alphaSecond),
        Math.round(catPixels[fila][col][1] * alphaFirst + dogPixels[fila][col][1] * alphaSecond),
        Math.round(catPixels[fila][col][2] * alphaFirst + dogPixels[fila][col][2] * alphaSecond)
      ];
