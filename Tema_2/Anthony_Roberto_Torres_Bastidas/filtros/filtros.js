...
/**
 * Funcion general para mapear los pixeles.
 */
function mapPixels(pixels, fn) {
...
/**
 * Funcion general para cambiar los pixeles a un color especifico
 */
function changePixels(pixels, color, dimFactor = 2) {
  const transform = {
    blue:   ([r, g, b]) => [0, 0, b],
    green:  ([r, g, b]) => [0, g, 0],
    red:    ([r, g, b]) => [r, 0, 0],
...
function blackAndWhiteConverter() {
  ...
  changePixels(pixels, "blackAndWhite");
...
function dimBrightness(dimFactor) {
  ...
  changePixels(pixels, "dimBrightness", dimFactor);
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let i = 0; i < catPixels.length; i ++) {
    ...
    for (let j = 0; j < catPixels[i].length; j ++) {
      let catPxl = catPixels[i][j];
      let dogPxl = dogPixels[i][j];
      let r = Math.floor(catPxl[0] * alphaFirst + dogPxl[0] * alphaSecond);
      let g = Math.floor(catPxl[1] * alphaFirst + dogPxl[1] * alphaSecond);
      let b = Math.floor(catPxl[2] * alphaFirst + dogPxl[2] * alphaSecond);
      pixels[i][j] = [r, g, b];
    }
  }
