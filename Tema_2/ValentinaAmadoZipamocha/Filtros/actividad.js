...
function scaleDown() {
  ...
  for (let i = 0; i < pixels.length; i += 2) {
    let newrow = [];
    let newcol = pixels[i];
    for (let j = 0; j < newcol.length; j += 2) {
      newrow.push(newcol[j]);
    }
...
function merge(alphaFirst, alphaSecond) {
  ...
  for (let a = 0; a < catPixels.length; a++) {
    let newRow = [];
    for (let b = 0; b < catPixels[a].length; b++) {
      let newPixel = [];
      for (let c = 0; c < 3; c++) {
        let addValue = Math.floor(catPixels[a][b][c] * alphaFirst + dogPixels[a][b][c] * alphaSecond);
        newPixel.push(addValue);
      }
