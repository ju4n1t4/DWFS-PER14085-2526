...
function greyConverter() {
    ...
    for (let i = 0; i < width; i++){
        for (let j = 0; j < height; j++){
            let px = pixels[i][j];
            let mean = Math.round((px[0] + px[1] + px[2]) / 3);
            px[0] = mean;
            px[1] = mean;
            px[2] = mean;
        }
...
function merge(alphaFirst, alphaSecond) {
    ...
    let pixels = new Array(width).fill(0).map(() =>
        new Array(height).fill(0).map(() => [0, 0, 0])
    );
    ...
    for (let i = 0; i < width; i++){
        for (let j = 0; j < height; j++){
            ...
            let px = pixels[i][j];
            let cat_px = catPixels[i][j];
            let dog_px = dogPixels[i][j];

            px[0] = alphaFirst * cat_px[0] + alphaSecond * dog_px[0];
            px[1] = alphaFirst * cat_px[1] + alphaSecond * dog_px[1];
            px[2] = alphaFirst * cat_px[2] + alphaSecond * dog_px[2];
        }
    }

    dogHandler.savePixels(pixels, outputPath, width, height);
}
