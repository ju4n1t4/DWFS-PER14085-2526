...
const ImageHandler = require('./js/ImageHandler')
...
function redConverter() {
  ...
  // Recorremos filas y columnas usando for...of
...
function scaleDown() {
  ...
  let rowIndex = 0;
  for (const row of pixels) {
    if (rowIndex % 2 === 0) {  // equivale a i += 2
      const newRow = [];

      let colIndex = 0;
      for (const pixel of row) {
        if (colIndex % 2 === 0) { // equivale a j += 2
          newRow.push(pixel);
        }
        colIndex++;
      }
...
function merge(alphaFirst, alphaSecond) {
  ...
  let rowIndex = 0;
  for (const catRow of catPixels) {
    if (rowIndex >= height) break;   

    const dogRow = dogPixels[rowIndex];
    const newRow = [];

    let colIndex = 0;
    for (const cPixel of catRow) {
      if (colIndex >= width) break;

      const dPixel = dogRow[colIndex];
      const r = Math.round(cPixel[0] * alphaFirst + dPixel[0] * alphaSecond);
      const g = Math.round(cPixel[1] * alphaFirst + dPixel[1] * alphaSecond);
      const b = Math.round(cPixel[2] * alphaFirst + dPixel[2] * alphaSecond);
      newRow.push([r, g, b]);
      colIndex++;
