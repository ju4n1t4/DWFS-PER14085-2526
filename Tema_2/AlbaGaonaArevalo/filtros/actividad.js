...
function redConverter() {
    ...
    let filas = handler.shape[0];
    let columnas = handler.shape[1];

    for (let i = 0; i < filas; i++) {
        for (let j = 0; j < columnas; j++) {
            pixels[i][j][1] = pixels[i][j][2] = 0;
        }
    }
...
function greyConverter() {
    ...
    for (let i = 0; i < filas; i++) {
        for (let j = 0; j < columnas; j++) {
            ...
            pixels[i][j][0] = pixels[i][j][1] = pixels[i][j][2] = media;
        }
    }
...
function merge(alphaFirst, alphaSecond) {
    ...
    let filas = catHandler.shape[0];
    let columnas = catHandler.shape[1];

    for (let i = 0; i < filas; i++) {
        let nuevaFila = [];
        for (let j = 0; j < columnas; j++) {
            dogPixels[i][j][0] = dogPixels[i][j][0] * alphaFirst;
            dogPixels[i][j][1] = dogPixels[i][j][1] * alphaFirst;
            dogPixels[i][j][2] = dogPixels[i][j][2] * alphaFirst;
            catPixels[i][j][0] = catPixels[i][j][0] * alphaSecond;
            catPixels[i][j][1] = catPixels[i][j][1] * alphaSecond;
            catPixels[i][j][2] = catPixels[i][j][2] * alphaSecond;
            let pixel = [catPixels[i][j][0] + dogPixels[i][j][0], catPixels[i][j][1] + dogPixels[i][j][1], catPixels[i][j][2] + dogPixels[i][j][2]];
            nuevaFila.push(pixel);
        }
