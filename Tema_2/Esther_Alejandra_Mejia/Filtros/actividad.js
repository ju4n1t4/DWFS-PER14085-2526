 ...
 * Media es a la suma de los tres y el resultado lo dividimos en tres.
...
function scaleDown() {
  ...
  let filasPixels = pixels.length;
  let columnasPixels = pixels[0].length;

  let nuevaImagen = []

  for (let i = 0; i < filasPixels; i+= 2){
    let filaNueva = [];
    for (let j = 0; j < columnasPixels; j+= 2)
    {
        let nueva = pixels[i][j];
        filaNueva.push(nueva);
    }
    nuevaImagen.push(filaNueva);
  }
...
function dimBrightness(dimFactor) {
  ...
  for (let i = 0; i < filas; i++)
    ...
    for (let j = 0; j < columnas; j++)
      {
        let pixel = pixels[i][j];
        let nuevoR = Math.round((pixel[0])/dimFactor);
        let nuevoG = Math.round((pixel[1])/dimFactor);
        let nuevoB = Math.round((pixel[2])/dimFactor);
...
function merge(alphaFirst, alphaSecond) {
  ...
  let filas = catPixels.length;
  let columnas = catPixels[0].length;
    ...
    for (let j = 0; j < columnas; j++)
      {
        ...
        let nuevoR =  Math.round(pixelCat[0] * alphaFirst + pixelDog[0] * alphaSecond);
        let nuevoG =  Math.round(pixelCat[1] * alphaFirst + pixelDog[1] * alphaSecond);
        let nuevoB =  Math.round(pixelCat[2] * alphaFirst + pixelDog[2] * alphaSecond);

        let pixelMerge = [nuevoR, nuevoG, nuevoB];

        filaNueva.push(pixelMerge);
    }
    pixels.push(filaNueva);
  }
