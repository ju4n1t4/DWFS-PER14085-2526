...
function redConverter() {
  ...
  const redPixels = pixels.map(row => { return row.map(pixel => [pixel[0], 0, 0]) });
...
function greenConverter() {
  ...
  const grenPixels = pixels.map(row => { return row.map(pixel => [0, pixel[1], 0]) });

  handler.savePixels(grenPixels, outputPath);
}
...
function blueConverter() {
  ...
  const bluePixels = pixels.map(row => { return row.map(pixel => [0, 0, pixel[2]]) });
...
function greyConverter() {
  ...
  const grayPixels = pixels.map(row => {
  ...
  handler.savePixels(grayPixels, outputPath);
}
...
function blackAndWhiteConverter() {
  ...
  const blackAndWhitePixels = pixels.map(row => {
    return row.map(pixel => {
      const median = (pixel[0] + pixel[1] + pixel[2]) / 3;
      return median < 128 ? [0, 0, 0] : [255, 255, 255]
...
function scaleDown() {
  ...
  const [height, width, channels] = handler.getShape();
  ...
  for (let row = 0; row < height; row += 2) {
    for (let col = 0; col < width; col += 2) {
      ...
      for (let c = 0; c < channels; c++) {
        scaleDownPixels.push(pixels[idx + c]);
      }
    }
  }
...
function dimBrightness(dimFactor) {
  ...
  const dimBrightnessPixeles = pixels.map(row => { return row.map(pixel => [pixel[0] / dimFactor, pixel[1] / dimFactor, pixel[2] / dimFactor]) });

  handler.savePixels(dimBrightnessPixeles, outputPath);
}
...
function invertColors() {
  ...
  const invertColorsPixeles = pixels.map(row => { return row.map(pixel => [255 - pixel[0], 255 - pixel[1], 255 - pixel[2]]) });


  handler.savePixels(invertColorsPixeles, outputPath);
}
  ...
  let dogPixels = dogHandler.getPixels();
   ...
   const pixels = catPixels.map((row, i) =>
    row.map((catPixel, j) => {
      ...
      return [
        catPixel[0] * alphaFirst + dogPixel[0] * alphaSecond, // R
        catPixel[1] * alphaFirst + dogPixel[1] * alphaSecond, // G
        catPixel[2] * alphaFirst + dogPixel[2] * alphaSecond, // B
        catPixel[3] * alphaFirst + dogPixel[3] * alphaSecond  // A
      ];
    })
  );
